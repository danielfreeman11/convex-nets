...
%By forming 
%$$t \mapsto W_{k^*-1}(t)=U(t) S(t) V(t)^T$$
$W_{k^*-1}(t)$ has the property that $W_{k^*-1}(0) = W_{k^*-1}^A$, $W_{k^*-1}(1) = W_{k^*-1}^B$.  
%\begin{equation}
%\label{lem}
...
%\begin{eqnarray}
%\label{kem2}
%\lim_{t\to 0} \| W_{k^*-1}^A W_{k^*-1}(t)^{-1} - {\bf 1} \| &=& \lim_{t \to 0} \| V_A \left( S_A S_A(t)^{-1} - {\bf 1}_k \right) V_A^T  \| \nonumber \\
%&=& \lim_{t\to 0}  \|  S_A S_A(t)^{-1} - {\bf 1}_k  \| \nonumber \\
%&=& 0~,
%\end{eqnarray}
%because the singular values $S_A$ are bounded away from zero.
%Equations (\ref{kem}) and (\ref{kem2}) imply that 
%$\lim_{t \to 0} W_{k^*}(t) = W_{k^*}^A $, and 
