...
Suppose the result is true for $K-1$. Let $\theta^A = (W_1^A, \dots, W^A_K)$ and 
 ...
 Since $n_j \geq \min(n_1, n_K)$ for $j=2 \dots K-1$, we can find $k^*$ such that
...
between $\tilde{\theta}_1$ and $\tilde{\theta_2}$. Let $\tilde{W}_{k^*}(t)$ the corresponding 
 path projected in the  layer $k^*$. 
