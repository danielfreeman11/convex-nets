...
Finally, since the singular values $s_{A,1} \dots, s_{A,m}$, $s_{B,1} \dots, s_{B,m}$ are non-negative, we can also construct paths 
$t \mapsto S_A(t)$, $t \mapsto S_B(t)$ such that $S(t)$ is diagonal, strictly positive with $S_{\{A,B\}}(0)=S_{\{A,B\}}$, $S_{\{A,B\}}(1)={\bf 1}$ and 
$\text{rank}(S_{\{A,B\}}(t)) = m~\forall~t\in(0,1]$. Moreover, we construct them such that 
\begin{equation}
\label{gem}
\| S_{\{A,B\}}(t) \|^{-1} = \frac{1}{\min_i {S_{\{A,B\}}}_{i,i}(t) } = o(t^{-1})~,~t\to 0~.
\end{equation}

We consider the path 
\begin{equation}
t \mapsto~W_{k^*-1}(t) = \left\{ 
\begin{array}{rl}
U_A S_A(3t) V_A^T & \text{if } 0 \leq t \leq \frac{1}{3}~,\\
U(3t-1) V(3t-1)^T & \text{if } \frac{1}{3} \leq t \leq \frac{2}{3}~,\\
U_B S_B(3-3t) V_B^T & \text{if } \frac{2}{3} \leq t~. 
\end{array} \right.
\end{equation}
...
\end{equation}
thanks to the fact that $\text{rank}(W_{k^*-1}(t)) = m$ for all $t \in (0,1)$. 
It follows that
\begin{eqnarray*}
(1-t) \theta^A + t \theta^B &=& (1-t) W_{k^*}^A W_{k^*-1}^A+ t W_{k^*}^B W_{k^*-1}^B \\
 &=& (1-t) W_{k^*}^A C^A(t) W_{k^*-1}(t) + t W_{k^*}^B C^B(t) W_{k^*-1}(t) \\
  ...
  &=& W_{k^*}(t)  W_{k^*-1}(t)~,
\end{eqnarray*}
by renaming $W_{k^*}(t) =  \left((1-t) W_{k^*}^A C^A(t) + t W_{k^*}^B C^B(t) \right)$. 

Finally, we need to show that the paths $W_{k^*}(t)$, $W_{k^*-1}(t)$ are continuous.
...
$$C^A(t) = W_{k^*-1}^A W_{k^*-1}(t)^{-1}~,~C^B(t) = W_{k^*-1}^B W_{k^*-1}(t)^{-1}~,\text{ so}$$
$$W_{k^*}(t) = (1-t) W_{k^*}^A W_{k^*-1}^A W_{k^*-1}(t)^{-1} +  t W_{k^*}^B W_{k^*-1}^B W_{k^*-1}(t)^{-1}~. $$
Consider first the case $t \to 0$.
Since $\|W_{k^*}^B W_{k^*-1}^B \| < \infty$ and $\|W_{k^*-1}(t)^{-1}\| = \| S_A(t) \|^{-1} = o(t^{-1})$, 
we have 
$$\lim_{t \to 0} \|  t W_{k^*}^B W_{k^*-1}^B W_{k^*-1}(t)^{-1} \| =0~.$$  
Denote by $k$ the rank of $W_{k^*-1}^A$, and by ${\bf 1}_k$ the diagonal matrix
with $k$ ones in the diagonal. Then 
\begin{eqnarray*}
\lim_{t\to 0} \| W_{k^*-1}^A W_{k^*-1}(t)^{-1} - V_A^{(k)} (V_A^{(k)})^T  \| &=& \lim_{t \to 0} \| V_A \left( S_A S_A(t)^{-1} - {\bf 1}_k \right) V_A^T  \| \\
\lim_{t\to 0}  \|  S_A S_A(t)^{-1} - {\bf 1}_k  \| &=& 0~,
\end{eqnarray*}
which implies $\lim_{t \to 0} W_{k^*}(t) = W_{k^*}^A $.
