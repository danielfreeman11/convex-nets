...
It has been shown in \cite{ganguli} and \cite{linearcase} that in this case, 
every local minima is a global minima.  
For completeness, we provide here an alternative proof of that result that uses
a simpler argument.
...
$$F( t \theta^A + (1-t) \theta^B) \leq t F(\theta^A) + (1-t) F(\theta^B) \leq \lambda~,$$
...
such that (i) $W_{K-1}(0) = W_{K-1}^A$, $W_{K-1}(1) = W_{K-1}^B$, 
(ii) $W_{K}(0) = W_{K}^A$, $W_{K}(1) = W_{K}^B$, and 
...
\end{equation}
to be the oracle risk using $m$ hidden units and using Ridge regression. 
...
In order to control the connectedness, we will a last definition. Given a 
...
\end{equation}
\begin{equation}
\epsilon = \inf_{n, \alpha} \left(\max \left\{ e(n), \delta_{W_1^A}(M, 0; M ) , \delta_{W_1^A}(M-n, \alpha; M ) ,  \delta_{W_1^B}(M, 0; M ) , \delta_{W_1^B}(M-n, \alpha; M ) \right\} + f(\alpha, M) \right)~,
\end{equation}
with 
$$f(\alpha, M) = C_1 \alpha  + C_2 M \alpha^2 + O(\alpha^2)~,$$
where $C_1$ and $C_2$ are absolute constants depending only on $\kappa$ and $P$.
\end{theorem}
